  ...
  switch (action.type) {
    case selectedConstants.SET_ALL:
      const result = {
        ...
        questionBy : action.data.questionBy ? action.data.questionBy : initialState.questionBy,
        geography : action.data.geography ? action.data.geography : initialState.geography,
